...
- **Hardware Collector System (Hardware Mode):** Real hardware data collection via companion Python scripts that run on the user's local machine. Scripts interface with actual hardware (WiFi adapters like Alfa AC-1000, Bluetooth adapters, etc.) and push scan results to the cloud app via `POST /api/collector/push` authenticated with Bearer API keys. Three collector scripts available: WiFi (`sigint_collector.py`), Bluetooth (`sigint_bluetooth_collector.py`), and Multi-Protocol (`sigint_multi_collector.py`). Scripts support Windows/macOS/Linux, GPS tagging, configurable intervals, and cross-platform hardware detection. API keys managed via Settings page. Dashboard has Hardware/Phone/Simulation mode toggle. macOS support uses CoreWLAN framework (via pyobjc) for WiFi scanning since Apple removed the airport utility in Sonoma 14.4+, and bleak for Bluetooth with UUID address handling (Apple privacy). All scripts support `--setup` flag for dependency diagnostics.
