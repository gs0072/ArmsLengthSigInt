...
- **Linux Hardware Scanner Service (server/services/linux-scanner.ts):** Background service that runs continuous passive scans using native Linux tools: `hcitool`/`bluetoothctl` for BLE, `iwconfig`/`iw` for WiFi, `rtl_sdr` for SDR, and `gpsd` for GPS. Auto-discovers devices and saves them with GPS coordinates. Controlled via Dashboard start/stop monitoring. API: `GET /api/scanner/status`, `POST /api/scanner/start`, `POST /api/scanner/stop`, `POST /api/scanner/manual-scan`.
